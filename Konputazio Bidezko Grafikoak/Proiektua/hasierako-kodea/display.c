...
void set_light_positions(){
    ...
    GLfloat scp[4]={_selected_camera->MZptr->M[12], _selected_camera->MZptr->M[13], _selected_camera->MZptr->M[14], 1}; 
    GLfloat scd[3]={-_selected_camera->MZptr->M[8], -_selected_camera->MZptr->M[9], -_selected_camera->MZptr->M[10]};
    glLightfv(GL_LIGHT3, GL_POSITION, scp);
    glLightf(GL_LIGHT3, GL_SPOT_CUTOFF, 180);
    glLightfv(GL_LIGHT3, GL_SPOT_DIRECTION, scd);
}

int colorizeObject(object3d *obj, face * aurpegia){
    double A[16];
    double MCSRobj[16];
    double EcObj[16];
    ...
    e_obj.x = MCSRobj[0]*_selected_camera->MZptr->M[12] + MCSRobj[4]*_selected_camera->MZptr->M[13] +MCSRobj[8]*_selected_camera->MZptr->M[14] +MCSRobj[12]*_selected_camera->MZptr->M[15];
    e_obj.y = MCSRobj[1]*_selected_camera->MZptr->M[12] + MCSRobj[5]*_selected_camera->MZptr->M[13] +MCSRobj[9]*_selected_camera->MZptr->M[14] +MCSRobj[13]*_selected_camera->MZptr->M[15];
    e_obj.z = MCSRobj[2]*_selected_camera->MZptr->M[12] + MCSRobj[6]*_selected_camera->MZptr->M[13] +MCSRobj[10]*_selected_camera->MZptr->M[14] +MCSRobj[14]*_selected_camera->MZptr->M[15];
    ...
    a=v.x * aurpegia->v_norm.x + v.y * aurpegia->v_norm.y +v.z * aurpegia->v_norm.z;
    return (a>0);
}
...
void display(void) 
{
    ...
    if((viewpoint==1) && (_selected_object!=0)){
        M_partikularra_lortu(_selected_object->MZptr->M, n_M);
    }else{
        M_partikularra_lortu(_selected_camera->MZptr->M, n_M);
    }
    ...
    for (int i = 0; i < 8; i++){
            glPushMatrix();
            glMultMatrixf(global_lighting[i].m_obj);
            put_light(i);
            glPopMatrix();
    }
    glLoadMatrixd(n_M);
    ...
    while (aux_obj != 0) {
        ...
        glMaterialfv(GL_FRONT, GL_AMBIENT, aux_obj ->material->ambient);
        glMaterialfv(GL_FRONT, GL_DIFFUSE, aux_obj ->material->diffuse);
        glMaterialfv(GL_FRONT, GL_SPECULAR, aux_obj ->material->specular);
        glMaterialfv(GL_FRONT, GL_SHININESS, aux_obj ->material->shininess);

        /* Draw the object; for each face create a new polygon with the corresponding vertex */
        glLoadMatrixd(n_M);
        glMultMatrixd(aux_obj->MZptr->M);
        for (f = 0; f < aux_obj->num_faces; f++) {
            glNormal3d(aux_obj->face_table[f].v_norm.x, aux_obj->face_table[f].v_norm.y, aux_obj->face_table[f].v_norm.z);
            glBegin(GL_POLYGON);
            /* if(colorizeObject(aux_obj, &(aux_obj->face_table[f]))){
