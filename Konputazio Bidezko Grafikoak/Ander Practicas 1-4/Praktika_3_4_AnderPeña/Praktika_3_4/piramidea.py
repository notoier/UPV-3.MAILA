...
N=24
triangeluZerrenda=piramidea_sortu(N, [1, 0.5, 0])
triangeluaParalelo=piramidea_paraleloan(triangeluZerrenda[0], pi/20, pi/4, [2, 0.5, 2], 5, 5, 1, 5)

Mald = []
for tri in triangeluaParalelo:
	column=np.column_stack((tri,[0,0,-1]))
	row = np.row_stack((column,[1,1,1,0]))
	inv = np.linalg.inv(row)
	Mald.append(inv)

m_zabal = 1000
piramide = np.ones((m_zabal, m_zabal, 1))

for i in range(m_zabal):
	for j in range(m_zabal):
		hurbilena=float('+inf')
		for tri in range(len(triangeluZerrenda[0])):
			koord_bariz = Mald[tri].dot([koadroa_xy(i,j)[0], koadroa_xy(i,j)[1], 0, 1])

			if (koord_bariz[0]>= 0 and koord_bariz[1]>=0 and koord_bariz[2]>=0 and koord_bariz[3]<hurbilena ):
				hurbilena = -koord_bariz[3]
				piramide[i,j] = triangeluZerrenda[1][tri]
plt.imshow(piramide)